...
function setupRequestHandlers(win2) {
  ipcMain.handle(
    "http-request",
    async (_, config) => {
      try {
        ...
        if ((body == null ? void 0 : body.stream) === true) {
          ...
          response2.data.on("data", (chunk) => {
            ...
            win2.webContents.send("update-message", str);
          });
          return { success: true };
        }
...
function createWindow() {
  ...
  win.webContents.on("did-finish-load", () => {
    win == null ? void 0 : win.webContents.send("main-process-message", `通信正常：${(/* @__PURE__ */ new Date()).toLocaleString()}`);
  });
